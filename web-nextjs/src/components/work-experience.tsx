...
const yearRanges: Record<string, { start: string; end: string; location: string }> = {
...
export default function WorkExperience() {
  return (
    <section className="py-24 max-md:py-16 border-t border-rule-strong" id="experience">
          ...
          <div>
            ...
            <FadeIn delay={60}>
              <h2 className="text-[clamp(28px,4vw,44px)] font-medium tracking-[-0.02em] leading-[1.05] mt-2">
                Over a decade of high-impact engineering<br />across startups and enterprise platforms.
              </h2>
            </FadeIn>
          </div>
          <FadeIn delay={120}>
          ...
          <div className="border-t border-rule">
            {work.map(({ company, roles }) => {
              ...
              return (
                ...
                >
                  <div className="font-[family-name:var(--mono)] text-xs tracking-[0.04em] pt-[3px]">
                    {range?.start} <span className="text-muted">— {range?.end}</span>
                  </div>
                  <div>
                    <div className="flex justify-between items-baseline gap-4">
                      <span className="text-lg font-medium tracking-[-0.015em]">{company}</span>
                      <span className="font-[family-name:var(--mono)] text-[11px] text-muted tracking-[0.04em] whitespace-nowrap">
                        {range?.location}
                      </span>
                    </div>
                    {roles.map(({ title, start, end, isCurrent, responsibilities }, ri) => (
                      <div
                        key={title}
                        className={`mt-3.5 ${ri > 0 ? "pt-[18px] border-t border-rule" : ""}`}
                      >
                        <div className="flex justify-between items-baseline gap-3 mb-2.5">
                          <span className="text-sm font-medium tracking-[-0.005em]">{title}</span>
                          <span className="font-[family-name:var(--mono)] text-[11px] text-muted tracking-[0.04em] whitespace-nowrap">
                        ...
                        <ul className="list-none p-0 m-0 grid gap-2">
                          {responsibilities.map((item) => (
                            <li
                              key={item}
                              className="text-[13.5px] leading-[1.55] pl-5 relative before:content-['—'] before:absolute before:left-0 before:top-0 before:text-muted before:font-[family-name:var(--mono)] before:text-xs"
                            >
                              {item}
                            </li>
                          ))}
