    ...
    <div>
      ...
      <MarginRow vertical={20}>
        <Col xs={12} md={6}>
          ...
          <Paragraph>The previous iteration of the usage page was developed in PHP using the Code Igniter framework and has not been updated since 2015.  We took the opportunity to redesign the usage page while migrating from PHP to Javascript.</Paragraph>
          ...
          <Paragraph>To further complicate the project, we had to consume many legacy upstream service calls to compose data structures to fit our designs.  For example, we would have to make sequential service calls and then manually stitch the data together based on identifiers from the resulting payloads.  This increased the complexity of our API more than we would have wanted.  Many of these complexities can be resolved at the database level.</Paragraph>
          <Paragraph>Moreover, these service calls had high response times.  We decided to load different portions of the page asynchronously to increase the perceived performance
          </Paragraph>
        </Col>
        <Col xs={0} md={6}>
          <ImageWithShadow
